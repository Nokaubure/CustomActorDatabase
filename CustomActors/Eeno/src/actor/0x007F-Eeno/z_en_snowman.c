...
#define ACT_ID 0x007F
#define OBJ_ID 0x007D
...
typedef enum {
    ...
    /* 0x6 */ EN_SNOWMAN_DMGEFF_BLUE_LIGHT_ORB = 0x6,
...
static DamageTable sDamageTable = {
    ...
    /* Slingshot      */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Explosive      */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Boomerang      */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Normal arrow   */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Hammer swing   */ DMG_ENTRY(4, EN_SNOWMAN_DMGEFF_NONE),
    /* Hookshot       */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_HOOKSHOT),
    /* Kokiri Sword   */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Master sword   */ DMG_ENTRY(2, EN_SNOWMAN_DMGEFF_NONE),
    /* Giant's Knife  */ DMG_ENTRY(4, EN_SNOWMAN_DMGEFF_NONE),
    /* Fire arrow     */ DMG_ENTRY(8, EN_SNOWMAN_DMGEFF_MELT),
    /* Ice arrow      */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Light arrow    */ DMG_ENTRY(2, EN_SNOWMAN_DMGEFF_LIGHT_ORB),
    /* Unk arrow 1    */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Unk arrow 2    */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_STUN),
    /* Unk arrow 3    */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Fire magic     */ DMG_ENTRY(8, EN_SNOWMAN_DMGEFF_MELT),
    /* Ice magic      */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_NONE),
    /* Light magic    */ DMG_ENTRY(4, EN_SNOWMAN_DMGEFF_LIGHT_ORB),
    /* Shield         */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_NONE),
    /* Mirror Ray     */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_NONE),
    /* Kokiri spin    */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
    /* Giant spin     */ DMG_ENTRY(4, EN_SNOWMAN_DMGEFF_NONE),
    /* Master spin    */ DMG_ENTRY(2, EN_SNOWMAN_DMGEFF_NONE),
    /* Kokiri jump    */ DMG_ENTRY(2, EN_SNOWMAN_DMGEFF_NONE),
    /* Giant jump     */ DMG_ENTRY(8, EN_SNOWMAN_DMGEFF_NONE),
    /* Master jump    */ DMG_ENTRY(4, EN_SNOWMAN_DMGEFF_NONE),
    /* Unknown 1      */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_NONE),
    /* Unblockable    */ DMG_ENTRY(0, EN_SNOWMAN_DMGEFF_NONE),
    /* Hammer jump    */ DMG_ENTRY(8, EN_SNOWMAN_DMGEFF_NONE),
    /* Unknown 2      */ DMG_ENTRY(1, EN_SNOWMAN_DMGEFF_NONE),
};
...
static InitChainEntry sInitChain[] = {
    ICHAIN_S8(naviEnemyId, 0xB0, ICHAIN_CONTINUE),
...
void EnSnowman_Init(Actor* thisx, PlayState* play) {
    ...
    if (EN_SNOWMAN_GET_TYPE(thisx) < EN_SNOWMAN_TYPE_SMALL_SNOWBALL) {
        ...
        Collider_SetCylinder(play, &this->collider, thisx, &sEenoCylinderInit);
        if (EN_SNOWMAN_GET_TYPE(thisx) == EN_SNOWMAN_TYPE_LARGE) {
            thisx->flags |= ACTOR_FLAG_10;
            Actor_SpawnAsChild(&play->actorCtx, thisx, play, ACT_ID, thisx->world.pos.x, thisx->world.pos.y,
                               thisx->world.pos.z, 0, 0, 0, EN_SNOWMAN_TYPE_SPLIT);
            thisx->parent = Actor_Spawn(&play->actorCtx, play, ACT_ID, thisx->world.pos.x,
                                                          thisx->world.pos.y, thisx->world.pos.z, 0, 0, 0,
                                                          EN_SNOWMAN_TYPE_SPLIT);
        ...
        Collider_InitCylinder(play, &this->collider);
        Collider_SetCylinder(play, &this->collider, thisx, &sSnowballCylinderInit);
...
void EnSnowman_SpawnCircularDustEffect(EnSnowman* this, PlayState* play) {
    ...
    for (i = 0; i < 16; i++) {
        ...
        func_8002829C(play, &pos, &sDustVelocity, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor,
...
void EnSnowman_SpawnBigSnowballFragmentEffects(EnSnowman* this, PlayState* play) {
    ...
    for (i = 0; i < 15; i++) {
        ...
        speedXZ = Rand_ZeroFloat(3.0f) + 8.0f;
        velocity.x = (speedXZ * Math_CosS(altitude)) * Math_SinS(azimuth);
        velocity.y = speedXZ * Math_SinS(altitude);
        velocity.z = (speedXZ * Math_CosS(altitude)) * Math_CosS(azimuth);
        ...
        EffectSsHahen_Spawn(play, &pos, &velocity, &sAccel, 0,
                            Rand_S16Offset((((i % 3) * 50) + 50), (((i % 3) * 25) + 25)), OBJ_ID, 20,
                            sSnowballFragmentDLs[i % 3]);
    }

    func_8002829C(play, &this->snowballPos, &gZeroVec3f, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor, 1000, 150);
}
...
void EnSnowman_MoveSnowPile(EnSnowman* this, PlayState* play) {
    ...
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
        SoundEffect_PlayHeld(SOUND_ENEMY_EENO_MOVE, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    } else {
        SoundEffect_PlayHeld(SOUND_ENEMY_EENO_MINI_MOVE, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    }
    ...
    } else if ((this->work.timer == 0) && (fabsf(this->actor.yDistToPlayer) < 60.0f) &&
...
void EnSnowman_SetupEmerge(EnSnowman* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_SURFACE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnSnowman_ReadySnowball(EnSnowman* this, PlayState* play) {
    ...
    if ((EN_SNOWMAN_GET_TYPE(&this->actor) != EN_SNOWMAN_TYPE_LARGE) && this->isHoldingSnowball &&
        ...
        func_8002829C(play, &pos, &sDustVelocity, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor, 500, 30);
        ...
        if ((this->skelAnime.curFrame > 3.0f) && (this->skelAnime.curFrame < 14.0f) &&
            ...
            pos.z = (this->actor.world.pos.z + (70.0f * Math_CosS(this->actor.shape.rot.y))) +
                    Rand_CenteredFloat(40.0f);
            func_8002829C(play, &pos, &sDustVelocity, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor, 1000, 150);
        }
    }
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
            SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_HOLD, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        } else {
            SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_MINI_HOLD, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        }
...
void EnSnowman_ThrowSnowball(EnSnowman* this, PlayState* play) {
    ...
    Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xA, 0x1000);
    if (SkelAnime_Update(&this->skelAnime)) {
        if ((this->work.snowballsToThrowBeforeIdling != 0) &&
            !(player->stateFlags1 & PLAYER_STATE1_23)) {
            EnSnowman_SetupReadySnowball(this);
        } else {
        ...
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
            ...
            SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_THROW, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        } else {
            params = EN_SNOWMAN_TYPE_SMALL_SNOWBALL;
            SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_MINI_THROW, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        }

        Actor_Spawn(&play->actorCtx, play, ACT_ID, this->snowballPos.x, this->snowballPos.y,
...
void EnSnowman_SetupSubmerge(EnSnowman* this, PlayState* play) {
    Animation_Change(&this->skelAnime, gEenoEmergeAnim, -1.0f, Animation_GetLastFrame(gEenoEmergeAnim), 0.0f,
                     ANIMMODE_ONCE, -3.0f);
    EnSnowman_SpawnCircularDustEffect(this, play);
    SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_HIDE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnSnowman_SetupDamaged(EnSnowman* this) {
    ...
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
        SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_DAMAGE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    } else {
        SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_MINI_DAMAGE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    }
...
void EnSnowman_Damaged(EnSnowman* this, PlayState* play) {
    ...
    if (this->work.timer > 0) {
        ...
        SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_SPLIT, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        EnSnowman_SpawnCircularDustEffect(this, play);
...
void EnSnowman_Dead(EnSnowman* this, PlayState* play) {
    ...
    for (i = 0; i < 15; i++) {
        ...
        azimuth = (Rand_Next() >> 0x10);
        speedXZ = Rand_ZeroFloat(2.0f) + 4.0f;
        velocity.x = (speedXZ * Math_CosS(altitude)) * Math_SinS(azimuth);
        velocity.y = speedXZ * Math_SinS(altitude);
        velocity.z = (speedXZ * Math_CosS(altitude)) * Math_CosS(azimuth);
        ...
        EffectSsHahen_Spawn(play, &pos, &velocity, &sAccel, 0,
                            Rand_S16Offset((((i % 3) * 20) + 20), (((i % 3) * 10) + 10)), OBJ_ID, 20,
                            sSnowballFragmentDLs[i % 3]);
    }

    func_8002829C(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor, 500, 30);
    SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_DEAD_BREAK, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x60);
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SPLIT) {
        EnSnowman_SetupSplitDoNothing(this);
    } else {
...
void EnSnowman_AbsorbEeno(EnSnowman* smallerEeno, EnSnowman* largerEeno) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_EENO_UNITE, 1.0f, 1.0f, &largerEeno->actor.world.pos, 0.0f, 1000.0f, &largerEeno->actor);
...
void EnSnowman_UpdateDamage(EnSnowman* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if ((this->actor.colChkInfo.damageEffect != EN_SNOWMAN_DMGEFF_HOOKSHOT) ||
            ...
            if (this->actor.colChkInfo.damageEffect == EN_SNOWMAN_DMGEFF_BLUE_LIGHT_ORB) {
                this->drawDmgEffScale = 0.55f;
                this->drawDmgEffAlpha = 4.0f;
    ...
    } else if ((this->actor.colChkInfo.health != 0) && (play->actorCtx.unk_02 != 0) && (this->actor.xzDistToPlayer <= 200.0f) && 
        ...
        } else if ((this->actionFunc == EnSnowman_MoveSnowPile) || (this->actionFunc == EnSnowman_Combine)) {
            EnSnowman_SetupEmerge(this, play);  
        }
        EnSnowman_SetupDamaged(this);
    }
        ...
        if (this->actionFunc != EnSnowman_Kill) {
            ...
            if ((this->actor.floorPoly != NULL) && ((this->actor.floorPoly->normal.y * SHT_MINV) < 0.7f)) {
                ...
                if (!this->turningOnSteepSlope) {
                    this->snowPileTargetRotY = Math_Atan2S(this->actor.floorPoly->normal.z * SHT_MINV,
...
void EnSnowman_UpdateSnowball(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SMALL_SNOWBALL) {
            scale = 10;
            for (i = 0; i < 3; i++) {
                EffectSsHahen_SpawnBurst(play, &thisx->world.pos, 5.0f, 0, scale, scale >> 1, 3, OBJ_ID, 20,
                                         sSnowballFragmentDLs[i]);
                scale *= 2;
            }

            func_8002829C(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, &sDustPrimColor, &sDustEnvColor, 500,
                          30);
            SoundEffect_PlayOneshot(SOUND_MM_SNOWBALL_BREAK_SMALL, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        } else {
            ...
            Math_Vec3f_Copy(&this->snowballPos, &this->actor.world.pos);
            EnSnowman_SpawnBigSnowballFragmentEffects(this, play);
            SoundEffect_PlayOneshot(SOUND_MM_SNOWBALL_BREAK_LARGE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        }

        SfxSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_IT_REFLECTION_WOOD);
        Actor_Kill(&this->actor);
        return;
    }
...
void EnSnowman_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if ((limbIndex == EENO_LIMB_RIGHT_HAND) && (this->isHoldingSnowball == true)) {
        ...
        gSPMatrix(&gfx[0], Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
