...
#define ACT_ID 0x0205
#define ACT2_ID 0x0206
#define ACT3_ID 0x0207
...
static DamageTable sDamageTable = {
    ...
    /* Hammer swing   */ DMG_ENTRY(1, EYEGORE_DMGEFF_NONE),
    /* Hookshot       */ DMG_ENTRY(1, EYEGORE_DMGEFF_NONE),
    /* Kokiri Sword   */ DMG_ENTRY(0, EYEGORE_DMGEFF_IMMUNE_0),
    /* Master Sword   */ DMG_ENTRY(0, EYEGORE_DMGEFF_IMMUNE_F),
    /* Giant's Knife  */ DMG_ENTRY(0, EYEGORE_DMGEFF_IMMUNE_F),
    ...
    /* Giant jump     */ DMG_ENTRY(0, EYEGORE_DMGEFF_IMMUNE_0),
    /* Master jump    */ DMG_ENTRY(0, EYEGORE_DMGEFF_IMMUNE_0),
        ...
        if (this->laserState == EYEGORE_LASER_OFF) {
            ...
            if ((this->actionTimer == 0) && BgCheck_EntityLineTest1(&play->colCtx, &this->laserCollider.dim.quad[3],
                ...
                if ((this->actor.world.pos.y - 50.0f <= player->actor.world.pos.y) &&
                    ...
                    for (i = 0; i < 3; i++) {
                        Actor_Spawn(&play->actorCtx, play, ACT2_ID, stonePos.x, stonePos.y, stonePos.z, 0,
...
void EnEgol_Slam(EnEgol* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 17.0f)) {
        ...
        Audio_PlayActorSfx2(&this->actor, NA_SE_EV_EXPLOSION);
        SoundEffect_PlayOneshot(SOUND_ENEMY_EYEROCK_POUND, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        ...
        if (this->actor.floorBgId == BGCHECK_SCENE) {
            ...
            for (i = 0; i < spawnCount; i++) {
                Actor_Spawn(&play->actorCtx, play, ACT2_ID, spawnPos.x, spawnPos.y, spawnPos.z, 0,
...
void EnEgol_Punch(EnEgol* this, PlayState* play) {
    ...
    if (curFrame >= this->animEndFrame) {
        ...
        if ((this->bodyCollider.elements[0].info.toucherFlags & TOUCH_HIT) ||
            ...
            this->hitPlayer = true;
            func_8002F6D4(play, &this->actor, 10.0f, this->actor.home.rot.y, 10.0f, 0);
        }
...
void EnEgol_Damaged(EnEgol* this, PlayState* play) {
    ...
    if (curFrame >= this->animEndFrame) {
        if (this->actor.colChkInfo.health > 0) {
            ...
            //SoundEffect_PlayOneshot(SOUND_ENEMY_DEATH_ARMOS_DIE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
            this->actor.flags |= ACTOR_FLAG_27;
