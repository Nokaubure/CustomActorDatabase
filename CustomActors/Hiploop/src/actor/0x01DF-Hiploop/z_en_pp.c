...
typedef enum {
    ...
    /* 0x6 */ EN_PP_DMGEFF_BLUE_LIGHT_ORB,       // Damages and surrounds the Hiploop with blue light orbs
...
static DamageTable sDamageTable = {
    ...
    /* Slingshot      */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Explosives     */ DMG_ENTRY(1, EN_PP_DMGEFF_KNOCK_OFF_MASK),
    /* Boomerang      */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Normal arrow   */ DMG_ENTRY(2, EN_PP_DMGEFF_DAMAGE),
    /* Hammer swing   */ DMG_ENTRY(4, EN_PP_DMGEFF_KNOCK_OFF_MASK),
    /* Hookshot       */ DMG_ENTRY(1, EN_PP_DMGEFF_HOOKSHOT),
    /* Kokiri sword   */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Master sword   */ DMG_ENTRY(2, EN_PP_DMGEFF_DAMAGE),
    /* Giant's Knife  */ DMG_ENTRY(4, EN_PP_DMGEFF_DAMAGE),
    /* Fire arrow     */ DMG_ENTRY(4, EN_PP_DMGEFF_FIRE),
    /* Ice arrow      */ DMG_ENTRY(4, EN_PP_DMGEFF_FREEZE),
    /* Light arrow    */ DMG_ENTRY(8, EN_PP_DMGEFF_LIGHT_ORB),
    /* Unk arrow 1    */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Unk arrow 2    */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Unk arrow 3    */ DMG_ENTRY(1, EN_PP_DMGEFF_DAMAGE),
    /* Fire magic     */ DMG_ENTRY(4, EN_PP_DMGEFF_FIRE),
    /* Ice magic      */ DMG_ENTRY(4, EN_PP_DMGEFF_FREEZE),
    /* Light magic    */ DMG_ENTRY(8, EN_PP_DMGEFF_BLUE_LIGHT_ORB),
    /* Shield         */ DMG_ENTRY(0, EN_PP_DMGEFF_JUMP),
    /* Light ray      */ DMG_ENTRY(0, EN_PP_DMGEFF_JUMP),
    /* Kokiri spin    */ DMG_ENTRY(2, EN_PP_DMGEFF_DAMAGE),
    /* Giant spin     */ DMG_ENTRY(8, EN_PP_DMGEFF_DAMAGE),
    /* Master spin    */ DMG_ENTRY(4, EN_PP_DMGEFF_DAMAGE),
    /* Kokiri jump    */ DMG_ENTRY(2, EN_PP_DMGEFF_DAMAGE),
    /* Giant jump     */ DMG_ENTRY(8, EN_PP_DMGEFF_DAMAGE),
    /* Master jump    */ DMG_ENTRY(4, EN_PP_DMGEFF_DAMAGE),
    /* Unknown 1      */ DMG_ENTRY(0, EN_PP_DMGEFF_JUMP),
    /* Unblockable    */ DMG_ENTRY(0, EN_PP_DMGEFF_JUMP),
    /* Hammer jump    */ DMG_ENTRY(8, EN_PP_DMGEFF_KNOCK_OFF_MASK),
    /* Powder Keg     */ DMG_ENTRY(8, EN_PP_DMGEFF_KNOCK_OFF_MASK),
};
...
static AnimationHeader* sAnimations[] = {
    0x0600a844,   0x06012ae4, 0x0600a304, 0x06001354, 0x06000e14,
    0x0600178c, 0x06009a74, 0x06012ae4,   0x0600a304, 0x06012ae4,
};
...
void EnPp_Init(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, 0x06012768, 0x0600a844, this->jointTable, this->morphTable,
                       26);
    ...
    if (this->action != EN_PP_ACTION_STUNNED_OR_FROZEN) {
        switch (this->animIndex) {
            ...
            case EN_PP_ANIM_WIND_UP:
                SoundEffect_PlayHeld(SOUND_ENEMY_HIPLOOP_CHARGE_START, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
                break;

            case EN_PP_ANIM_CHARGE:
                SoundEffect_PlayHeld(SOUND_ENEMY_HIPLOOP_CHARGE_LOOP, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
                break;

            case EN_PP_ANIM_ATTACK:
                if (Animation_OnFrame(&this->skelAnime, 0.0f)) {
                    SoundEffect_PlayOneshot(SOUND_ENEMY_HIPLOOP_CHARGE_END, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
                }
                break;
...
void EnPp_SetupDamaged(EnPp* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_HIPLOOP_HIT, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnPp_SetupDead(EnPp* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_HIPLOOP_DIE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnPp_Dead(EnPp* this, PlayState* play) {
    ...
    if ((this->action == EN_PP_ACTION_DEAD) && (isUnderWater || (this->timer == 1))) {
        ...
        //EffectSsKFire_Spawn(play, &this->bodyPartsPos, &vel, &accel, 40, 0);
    }
 ...
 */
void EnPp_Mask_Detach(EnPp* this, PlayState* play) {
    if (!CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_13) || (this->action == EN_PP_ACTION_MASK_DEAD)) {
        switch (this->actionVar.maskDetachState) {
            case EN_PP_MASK_DETACH_STATE_START:
                ...
                SoundEffect_PlayOneshot(SOUND_ENEMY_HIPLOOP_UNMASKED, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
                ...
                if (this->maskPos.y > 900.0f) {
                    if (this->maskVelocity.y > 0.0f) {
                        ...
                        for (i = 0; i < ARRAY_COUNT(sMaskFireVelocityAndAccel); i++) {
                            ...
                            EffectSsDeadDb_Spawn(play, &maskFirePos, &sMaskFireVelocityAndAccel[i],
                                          &sMaskFireVelocityAndAccel[i], 70, 0, 255, 255, 255, 255, 0, 0, 255, 1, 9, true);
                        }
        ...
        if (EN_PP_GET_TYPE(&this->actor) != EN_PP_TYPE_MASKED) {
            if ((this->action < EN_PP_ACTION_DAMAGED) && (this->action != EN_PP_ACTION_JUMP)) {
                ...
                if (this->actor.colChkInfo.damageEffect == EN_PP_DMGEFF_ELECTRIC_STUN) {
                    ...
                    } else if ((this->actor.colChkInfo.damageEffect == EN_PP_DMGEFF_BLUE_LIGHT_ORB) &&
                               ((((this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_SFX)) &&
                                 (this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX)) ||
                                (this->drawDmgEffTimer == 0))) {
                        /*Actor_Spawn(&play->actorCtx, play, 0x33B, this->actor.focus.pos.x,
...
s32 EnPp_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    ...
    if (this->action != EN_PP_ACTION_BODY_PART_MOVE) {
        ...
        if ((limbIndex == 8) && (EN_PP_GET_TYPE(&this->actor) != EN_PP_TYPE_MASKED)) {
            *dList = NULL;
        }
...
void EnPp_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (this->action != EN_PP_ACTION_BODY_PART_MOVE) {
        ...
        if ((limbIndex == 1) || (limbIndex == 3) ||
            ...
            (limbIndex == 13) || (limbIndex == 16) ||
            (limbIndex == 18) || (limbIndex == 19) ||
            (limbIndex == 22) || (limbIndex == 24) ||
            (limbIndex == 25)) {
