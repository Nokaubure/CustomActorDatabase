...
#define ACT_ID 0x0017
#define OBJ_ID 0x0079
...
void EnGrasshopper_SetupFly_Still(EnGrasshopper* this);
void EnGrasshopper_Fly_Still(EnGrasshopper* this, PlayState* play);
...
typedef enum {
    ...
    /* 0x5 */ DRAGONFLY_DMGEFF_BLUE_LIGHT_ORB,  // Damages and surrounds the Dragonfly with blue light orbs
...
static DamageTable sDamageTable = {
    ...
    /* Slingshot      */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Explosive      */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Boomerang      */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Normal arrow   */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Hammer swing   */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Hookshot       */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_HOOK),
    /* Kokiri sword   */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Master sword   */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Giant's Knife  */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Fire arrow     */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_FIRE),
    /* Ice arrow      */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_FREEZE),
    /* Light arrow    */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_LIGHT_ORB),
    /* Unk arrow 1    */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Unk arrow 2    */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Unk arrow 1    */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Fire magic     */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_FIRE),
    /* Ice magic      */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_FREEZE),
    /* Light magic    */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_BLUE_LIGHT_ORB),
    /* Shield         */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_IMMUNE),
    /* Mirror ray     */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_IMMUNE),
    /* Kokiri spin    */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Giant spin     */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_NONE),
    /* Master spin    */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Kokiri jump    */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Giant jump     */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_NONE),
    /* Master jump    */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Unknown 1      */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_IMMUNE),
    /* Unblockable    */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_IMMUNE),
    /* Hammer jump    */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Powder Keg     */ DMG_ENTRY(8, DRAGONFLY_DMGEFF_NONE),
};
...
void EnGrasshopper_Init(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_Init(play, &this->skelAnime, 0x06003f00, 0x06000f9c, this->jointTable, this->morphTable,
                   DRAGONFLY_LIMB_MAX);
    ...
    //if (this->type = DRAGONFLY_TYPE_STATIONARY) {
    //    EnGrasshopper_SetupFly_Still(this);
    //} else {
...
static AnimationHeader* sAnimations[] = {
    0x060042f0, // DRAGONFLY_ANIM_RAISE_TAIL
    0x06004bc8, // DRAGONFLY_ANIM_LOWER_TAIL
    0x06000f9c,       // DRAGONFLY_ANIM_FLY
    0x0600066c,    // DRAGONFLY_ANIM_ATTACK
    0x0600524c,     // DRAGONFLY_ANIM_HOVER
    0x060047ac,    // DRAGONFLY_ANIM_DAMAGE
    0x060008d4,      // DRAGONFLY_ANIM_DEAD
    0x060009e0,      // DRAGONFLY_ANIM_FALL
};
...
void EnGrasshopper_Fly(EnGrasshopper* this, PlayState* play) {
    ...
    SoundEffect_PlayHeld(SOUND_ENEMY_DRAGONFLY_FLY, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    ...
    if ((Player_GetMask(play) != PLAYER_MASK_SKULL) && !this->shouldTurn &&
...
void EnGrasshopper_SetupFly_Still(EnGrasshopper* this) {
    ...
    this->baseFlyHeight = Rand_CenteredFloat(50.0f) + this->flyingHomePos.y;
    this->action = DRAGONFLY_ACTION_FLY;
    this->actionFunc = EnGrasshopper_Fly_Still;
}
...
void EnGrasshopper_SetupAttack(EnGrasshopper* this) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_DRAGONFLY_ATTACK, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnGrasshopper_Attack(EnGrasshopper* this, PlayState* play) {
    ...
    if ((this->collider.base.atFlags & AT_BOUNCED) ||
        ((player->stateFlags1 & PLAYER_STATE1_22) && (playerToHitPosDist <= 60.0f) &&
...
void EnGrasshopper_SetupDead(EnGrasshopper* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_DRAGONFLY_DIE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnGrasshopper_Fall(EnGrasshopper* this, PlayState* play) {
    ...
    if (isUnderWater || (BgCheck_SphVsFirstPoly(&play->colCtx, &this->tailTipPos, 10.0f))) {
        ...
        for (i = 0; i < ARRAY_COUNT(sFireVelocityAndAccel); i++) {
            ...
            EffectSsDeadDb_Spawn(play, &firePos, &sFireVelocityAndAccel[i], &sFireVelocityAndAccel[i], 100, 0, 255, 255, 255, 255, 0, 0, 255, 1, 9,
                                 true);
        }

        //SfxSource_PlaySfxEachFrameAtFixedWorldPos(play, &this->actor.world.pos, 10,
        //                                            NA_SE_EN_COMMON_EXTINCT_LEV - SFX_FLAG);
        SfxSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 10,
                                                    NA_SE_EN_EXTINCT);
...
void EnGrasshopper_UpdateDamage(EnGrasshopper* this, PlayState* play) {
    ...
    if ((this->collider.elements[0].info.bumperFlags & BUMP_HIT) ||
        ...
        if ((this->action != DRAGONFLY_ACTION_DAMAGED) && (this->action != DRAGONFLY_ACTION_DEAD) &&
            ...
            if (this->actor.colChkInfo.damageEffect == DRAGONFLY_DMGEFF_NONE) {
                ...
                Actor_Spawn(&play->actorCtx, play, 0x033B, this->actor.focus.pos.x, this->actor.focus.pos.y,
                            this->actor.focus.pos.z, 0, 0, 0, 0x04);
                attackDealsDamage = true;
            } else if (this->actor.colChkInfo.damageEffect == DRAGONFLY_DMGEFF_BLUE_LIGHT_ORB) {
                Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_GRAY, 255, COLORFILTER_BUFFLAG_OPA, 25);
                this->drawDmgEffTimer = 20;
    ...
    if (attackDealsDamage) {
        ...
        if (this->actor.colChkInfo.health > 0) {
            ...
            SoundEffect_PlayOneshot(SOUND_ENEMY_DRAGONFLY_HIT, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnGrasshopper_DrawEffects(EnGrasshopper* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->effects); i++, effect++) {
        if (effect->isEnabled) {
            ...
            SkinMatrix_SetRotateZYX(&mfRot, 0, 0, effect->yaw);
