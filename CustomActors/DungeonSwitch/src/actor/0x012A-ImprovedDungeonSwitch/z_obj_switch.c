...
New types (set Toggle to 1)
- 0005 Green/Yellow switch
- 0006 Targetable Green/Yellow switch
...
void ObjSwitch_Init(Actor* thisx, PlayState* play) {
    ...
    if (type == OBJSWITCH_TYPE_CRYSTAL2 || type == OBJSWITCH_TYPE_CRYSTAL2_TARGETABLE)
    {
        type -= 2;
        this->alternate = 1;
        this->dyna.actor.params = (this->dyna.actor.params & 0xFFF8) | type;
    }
    ...
    if ((this->dyna.actor.params & 7) == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        ...
        switch (this->dyna.actor.params >> 4 & 7) {
            case OBJSWITCH_SUBTYPE_ONCE:
                if (func_800435B4(&this->dyna)) { // edit
                    ObjSwitch_FloorPressInit(this);
                    ObjSwitch_SetOn(this, play);
                }
                break;
...
void ObjSwitch_CrystalOnInit(ObjSwitch* this) {
    ...
    this->crystalSubtype1texture = (this->alternate == 0 ) ? gCrstalSwitchBlueTex : (void*)0x050134A0; //edit 
