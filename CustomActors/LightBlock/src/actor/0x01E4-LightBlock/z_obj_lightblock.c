...
void func_80AF3910(ObjLightblock* this, PlayState* play) {
    ...
    s16 index = Object_GetIndex(&play->objectCtx, OBJECT_EFC_TW);
    if (index != -1 && Object_IsLoaded(&play->objectCtx, index))
        Actor_Spawn(&play->actorCtx, play, ACTOR_DEMO_EFFECT, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y,this->dyna.actor.world.pos.z, 0, 0, 0, 0x0018);
}

void ObjLightblock_InitDynaPoly(ObjLightblock* this, PlayState* play, CollisionHeader* collision, s32 flags) {
    s32 pad;
...
void ObjLightblock_Init(Actor* thisx, PlayState* play) {
    ...
    ObjLightblock_InitDynaPoly(this, play, gSunBlockCol, DPM_PLAYER);
...
void func_80AF3BA0(ObjLightblock* this, PlayState* play) {
        ...
        // "◯◯◯◯ Sun Block Attention Camera (frame counter  %d)\n"
