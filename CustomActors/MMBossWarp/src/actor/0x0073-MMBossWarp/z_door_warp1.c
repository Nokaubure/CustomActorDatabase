...
#define SWITCH_FLAG (this->dyna.actor.world.rot.x)
#define OBJ_ID 0x016E
#define ACT_ID 0x0083
...
void func_808B8A7C(DoorWarp1* this, PlayState* play) {
    SkelAnime_Init(play, &this->skelAnime, 0x06002ca8, 0x06001374, NULL, NULL, 0);
    Animation_ChangeImpl(&this->skelAnime, 0x06001374, 1.0f, 1.0f, 1.0f, 2, 40.0f, 1);
    this->unk_1C4 = 0;
    this->unk_1C6 = -140;
    this->unk_1C8 = -80;
    this->unk_1D0 = 0;
...
void func_808B8C48(DoorWarp1* this, PlayState* play) {
    ...
    if (((DOORWARP1_GET_FF(&this->dyna.actor) == ENDOORWARP1_FF_2)) ||
        ...
        params |= 6;
        Actor_SpawnAsChild(&play->actorCtx, &this->dyna.actor, play, ACT_ID, this->dyna.actor.world.pos.x,
...
void func_808B8E78(DoorWarp1* this, PlayState* play) {
    ...
    this->unk_1AC = 0.0f;
    this->unk_1A4 = 700.0f;
    /*if (CHECK_WEEKEVENTREG(WEEKEVENTREG_86_80)) {
        this->unk_1D4 = 0;
    ...
    //this->unk_1D4 = 0;
    //DoorWarp1_SetupAction(this, func_808B921C);
    if (Flags_GetSwitch(play, SWITCH_FLAG) || SWITCH_FLAG == 0x7F) {
        this->unk_1D4 = 0;
        DoorWarp1_SetupAction(this, func_808B921C);
    } else {
        DoorWarp1_SetupAction(this, func_808B9094);
...
*/
    ...
    //Audio_PlayActorSfx2(&this->dyna.actor, NA_SE_EV_WARP_HOLE_ENERGY - SFX_FLAG);
    SoundEffect_PlayHeld(SOUND_MM_BOSS_WARP_ENERGY, 1.0f, 1.0f, 1.0f, &this->dyna.actor.world.pos, 1.0f, 2000.0f, &this->dyna.actor);
}
...
void func_808B921C(DoorWarp1* this, PlayState* play) {
    ...
    if (func_808B866C(this, play) && !Play_InCsMode(play)) {
        if ((this->dyna.actor.parent->params & 0x00FF) != ENDOORWARP1_FF_3)
        {
            func_800B7298(play, &this->dyna.actor, PLAYER_CSACTION_WAIT);
            Message_StartTextbox(play, 0x1F2, &this->dyna.actor);
        ...
        else
        {
            ...
            player->unk_450.x = this->dyna.actor.world.pos.x;
            player->unk_450.z = this->dyna.actor.world.pos.z;
            this->unk_1CA = 1;
            DoorWarp1_SetupAction(this, func_808B9524);
        }
    }

    //Audio_PlayActorSfx2(&this->dyna.actor, NA_SE_EV_BOSS_WARP_HOLE - SFX_FLAG);
    SoundEffect_PlayHeld(SOUND_MM_BOSS_WARP, 1.0f, 1.0f, 1.0f, &this->dyna.actor.world.pos, 1.0f, 2000.0f, &this->dyna.actor);
}
...
void func_808B958C(DoorWarp1* this, PlayState* play) {
    ...
    Audio_PlayActorSfx2(&this->dyna.actor, NA_SE_EV_LINK_WARP - SFX_FLAG);
}
...
void func_808B96B0(DoorWarp1* this, PlayState* play) {
    Audio_PlayActorSfx2(&this->dyna.actor, NA_SE_EV_WARP_HOLE - SFX_FLAG);
...
void func_808B977C(DoorWarp1* this, PlayState* play) {
    ...
    if (func_808B866C(this, play) && !Play_InCsMode(play)) {
        ...
        func_800B7298(play, &this->dyna.actor, PLAYER_CSACTION_9);
        //OnePointCutscene_Init(play, 0x25E7, 999, &this->dyna.actor, CAM_ID_MAIN);
        player->unk_450.x = this->dyna.actor.world.pos.x;
        player->unk_450.z = this->dyna.actor.world.pos.z;
        this->unk_1CA = 1;
    ...
    if (D_808BC000 < this->unk_1D0) {
        if ((gSaveContext.nextCutsceneIndex == 0xFFEF) && (D_808BC000 < this->unk_1D0) &&
            ...
            if (DOORWARP1_GET_FF00_1(&this->dyna.actor) != 0xFF) {
                ...
                for (s32 i = 0; i < DOORWARP1_GET_FF00_3(&this->dyna.actor); i++) {
...
void func_808B9FD0(DoorWarp1* this, PlayState* play) {
    ...
    //} else {
        ...
        Audio_PlaySfxGeneral(NA_SE_EV_LINK_WARP, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        Animation_ChangeImpl(&this->skelAnime, 0x06001374, 1.0f, Animation_GetLastFrame(0x06001374),
                             Animation_GetLastFrame(0x06001374), 2, 40.0f, 1);
        this->unk_1CA = 50;
...
void func_808BA550(DoorWarp1* this, PlayState* play) {
    ...
    if (temp_f0 > 0.0f) {
        for (i = 0; i < ARRAY_COUNT(play->envCtx.adjLight1Color); i++) {
            play->envCtx.adjAmbientColor[i] = play->envCtx.adjFogColor[i] =
                play->envCtx.adjLight1Color[i] = -255.0f * temp_f0;
        }

        play->envCtx.adjFogNear = -500.0f * temp_f0;
...
*/
    ...
    if ((fabsf(this->dyna.actor.xzDistToPlayer) < phi_f2) &&
        ((player->actor.world.pos.y - 20.0f) < this->dyna.actor.world.pos.y) &&
        (this->dyna.actor.world.pos.y < (player->actor.world.pos.y + 20.0f))) {
        //csId = this->dyna.actor.csId;
        DoorWarp1_SetupAction(this, func_808BABF4);
...
void func_808BAE9C(DoorWarp1* this, PlayState* play) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x0A, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__));
    Matrix_Push();
    ...
    gSPSegment(POLY_XLU_DISP++, 0x09, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__));
    gSPDisplayList(POLY_XLU_DISP++, 0x060001a0);
...
void func_808BB4F4(DoorWarp1* this, PlayState* play2) {
    ...
    if (this->dyna.actor.rot.z != 0 && this->dyna.actor.rot.z <= 6) {
            sp60 = this->dyna.actor.rot.z;
    }
