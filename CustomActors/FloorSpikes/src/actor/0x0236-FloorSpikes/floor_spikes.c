...
#define ACT_ID 0x0236
#define OBJ_ID 0x020D
...
void FloorSpikes_Init(Actor* thisx, PlayState* play) {
    ...
    switch (SPIKE_SIZE(this)) {
        case SPIKE_SIZE_SMALL:
            CollisionHeader_GetVirtual(0x060000d8, &colHeader);
            break;
        case SPIKE_SIZE_MEDIUM:
            CollisionHeader_GetVirtual(0x060017d8, &colHeader);
            break;
        case SPIKE_SIZE_LARGE:
            CollisionHeader_GetVirtual(0x06003088, &colHeader);
            break;
    }
    ...
    if ((this->spikesActive) && (this->spikeProgress > 0.5f)) {
        ...
        if (player->actor.freezeTimer == 0) {
            ...
            if (DynaPolyActor_IsPlayerOnTop(&this->dyna)) {
                player->linearVelocity = CLAMP_MAX(player->linearVelocity, 3.0f);
            }
        }
    }
}
...
static Gfx* sBaseDLists[3] = { 0x06000ba8, 0x060022a8, 0x06003b58 };
static Gfx* sSpikesDLists[3] = { 0x060016c0, 0x06002f78, 0x06004b90 };
...
void FloorSpikes_Trigger(FloorSpikes* this, PlayState* play) {
    if (DECR(this->timer) == 0) {
        ...
        if (this->timer <= SPIKE_RETRACT_TIME) {
            Audio_PlayActorSfx2(&this->dyna.actor, NA_SE_IT_SWORD_PICKOUT - SFX_FLAG);
